...
I noticed that with 16 threads working, the CPU utilization would sometimes hit over 800% for the 8 cores I have. I ran a sanity check to ensure the bottleneck wasn't CPU or memory (too much memory can lead to memory swapping) by running similar tests with less records with less threads for just 1 minute, and the results below indicate that neither CPU nor memory limits are the problem.
